...
ПЕРЕД КАЖДЫМ ТЕКСТОМ напиши три правила:
...
ПРАВИЛО: Не придумывай факты. Только то что дал пользователь. Тире нигде не ставь. Продукт в конце как следствие."""
...
def get_generate_keyboard():
    return ReplyKeyboardMarkup([[KeyboardButton("Написать текст ✍️")]], resize_keyboard=True, one_time_keyboard=True)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привет! Я Сяолин — контент-ассистент Елены Амагыровой 🐼\n\nВыбери формат 👇",
...
async def get_format(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Выбери тему (или напиши несколько через запятую) 👇",
...
async def get_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Выбери цель контента 👇", reply_markup=get_goal_keyboard())
    return GOAL
...
async def get_goal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Выбери тональность (или напиши несколько) 👇", reply_markup=get_tone_keyboard())
    return TONE
...
async def get_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Напиши факты и детали из жизни Елены 👇\n\nТолько реальные — ничего не придумываю!"
    )
    return FACTS
...
async def get_stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    text = update.message.text
    user_data_store[user_id]['stop'] = '' if text == 'Написать текст ✍️' else text
...
async def retry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id in user_data_store and user_data_store[user_id].get('facts'):
        await generate_text(update, context, user_id)
    else:
